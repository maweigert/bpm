...
def focus_field_lattice(shape,units,lam,NAs=(.4,.5), slices = (-.9,0,.9),
                        n0=1., n_integration_steps = 100):
    """
    """

    p = OCLProgram(absPath("kernels/psf_lattice.cl"),build_options = str("-I %s -D INT_STEPS=%s"%(absPath("."),n_integration_steps)))

    NAs = np.array(NA)

    if len(NAs) != 2:
        raise ValueError("Nas should be a tuple of length 2")

    Nx, Ny, Nz = shape
    dx, dy, dz = units

    alpha = np.arcsin(NAs/n0)
    ...
    p.run_kernel("psf_lattice",u_g.shape[::-1],None,
                 ex_g.data,u_g.data,
