...
__kernel void debye_wolf_lattice(__global cfloat_t * Ex,
  ...
  for (int i_n = 0; i_n < Nks; i_n++) {
	...
	float y1 = y_pos0 - 4.*sigma;
	float y2 = y_pos0 + 4.*sigma;
	float dy = (y2-y1)/(INT_STEPS-1.f);
	  ...
	  float exfac = 1./sigma*exp(-(y_pos-y_pos0)*(y_pos-y_pos0)/2.f/sigma/sigma);

  	  float prefac = ((y_pos==y1)||(y_pos==y2))?.5f:1.f;

  	  prefac *= 1./theta*dy*exfac*sqrt(co_t)*si_t;
...
__kernel void debye_wolf_lattice_plane(__global cfloat_t * Ex,
  ...
  float dx_apo = (dist_bound<apo_bound)?(1.f*dist_bound/apo_bound):1.;
