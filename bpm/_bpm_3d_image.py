...
def _bpm_3d_image(size,
    ...
    program = OCLProgram(absPath("kernels/bpm_3d_image_kernels.cl"))
...
def _bpm_3d_image_split(size, units, lam = .5, u0 = None, dn = None,
                        n_volumes = 1,
                        n0 = 1.,
                        subsample=1,
                        return_scattering = False,
    ...
    for i in range(n_volumes):
        ...
        if i<n_volumes-1:
            res_part = _bpm_3d_image((Nx,Ny,i2-i1+1),
                               units = units,
                               lam = lam,
                               u0 = u0,
                               dn = dn[i1:i2+1,:,:],
            ...
            res_part = _bpm_3d_image((Nx,Ny,i2-i1),
                               units = units,
                               lam = lam,
                               u0 = u0,
                               dn = dn[i1:i2,:,:],
...
def bpm_3d_image_free(size, units, dz, lam = .5, u0 = None,
                n0 = 1., 
                use_fresnel_approx = False):
